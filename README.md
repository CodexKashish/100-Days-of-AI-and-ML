# 🧭 100 Days of AI & ML Journey
My daily progress log for mastering Python, SQL and Machine Learning.

Welcome to my technical expedition! I am a 1st-year B.Tech CSE student (2nd Sem) documenting my daily progress as I explore the world of Data Science and Artificial Intelligence.

## 📍 The Roadmap
- [ ] **Phase 1: Python Foundations** (Current)
- [ ] **Phase 2: SQL & Database Management**
- [ ] **Phase 3: Data Analysis (Pandas/NumPy)**
- [ ] **Phase 4: Machine Learning with Scikit-Learn**

## 📓 Daily Log
| Day | Topic | Project/Task |
| :--- | :--- | :--- |
| 01 | Inputs & Math | [Age to Days Calculator](./Day_01_Age_Calculator.py) |

---
*“Nature is the best engineer; I’m just learning to code like it.”*
